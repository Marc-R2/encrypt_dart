...
class RSAKeyGenerator implements KeyGenerator {
  ...
  static final BigInt _bigTwo = BigInt.from(2);

  static bool _testBit(BigInt i, int n) =>
      (i & (BigInt.one << n)) != BigInt.zero;

  /// return index of lowest 1-bit in x, x < 2^31
  static int _lbit(BigInt xRaw) {
    var x = xRaw;
  ...
  static bool _millerRabin(BigInt b, int tRaw) {
    var t = tRaw;
    // Implementation borrowed from bignum.BigIntegerDartvm.
    final n1 = b - BigInt.one;
    final k = _lbit(n1);
    if (k <= 0) return false;
    final r = n1 >> k;
    t = (t + 1) >> 1;
  ...
  static bool _isProbablePrime(BigInt b, int t) {
    // Implementation borrowed from bignum.BigIntegerDartvm.
    int i;
    final x = b.abs();
  ...
  static BigInt generateProbablePrime(
      int bitLength, int certainty, SecureRandom rnd) {
    if (bitLength < 2) {
      return BigInt.one;
    }
  ...
  AsymmetricKeyPair generateKeyPair() {
    ...
    final strength = _params.bitStrength;
    final pbitlength = (strength + 1) ~/ 2;
    final qbitlength = strength - pbitlength;
    final mindiffbits = strength ~/ 3;
